...
export function AvailableAppointments({
  ...
  return (
    <section className="panel" aria-labelledby="available-heading">
      <div className="section-heading">
        <div>
          <span className="step-label">Step 1</span>
          <h2 id="available-heading">Choose an appointment</h2>
        </div>
        <button
      ...
      {!isLoading && error === null && slots.length === 0 && (
        <p className="empty-message">
          There are no appointments available at the moment. Try refreshing the
          list later or use an assisted booking route.
        </p>
      )}

      {!isLoading && error === null && slots.length > 0 && (
        <fieldset className="slot-fieldset" disabled={disabled}>
          <legend className="visually-hidden">Available appointments</legend>
              ...
              <li key={slot.appointmentSlotId}>
                ...
                >
                  ...
                  <span>
                    ...
                    <span className="slot-clinician">{slot.clinicianName}</span>
